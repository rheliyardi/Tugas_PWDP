...
// 1. mysqli_query
$sql=$this->mysqli->query("SELECT * FROM guru") or die
($this->CekError());
...
// 1. mysqli_query
$sql=$this->mysqli->query("SELECT * FROM guru WHERE nip='".$nip."'") or die ($this->CekError());
...
$this->mysqli->query("UPDATE guru SET ".$script_update."
WHERE nip='".$nip."'") or die ($this->CekError());
// 4. tutup koneksi
$this->TutupKoneksi();
}
function hapus($nip){
// 1. Jalankan perintah delete query
$this->mysqli->query("DELETE FROM guru WHERE nip='$nip'");
// 2. tutup koneksi
$this->TutupKoneksi();
}
...
$sql_simpan="INSERT INTO guru (".$kolom_nya_baru.")VALUES (".$nilai_nya_baru.")";
